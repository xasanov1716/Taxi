...
class NotificationCubit extends Cubit<NotificationState> {
  ...
  Future<void> saveNotificationValues(
      List<String> keys, List<bool> values) async {
  ...
  Future<void> readNotificationValues(List<String> keys) async {
    ...
    for (int i = 0; i < keys.length; i++) {
      final value = StorageRepository.getBool(keys[i]) ?? false;
      values.add(value);
    }
    emit(state.copyWith(notificationValues: values, notificationNames: keys));
  }

  Future<void> updateNotificationValues(
      String key, bool value, List<String> keys) async {
