...
class SelectRequestDriver extends StatefulWidget {
  const SelectRequestDriver({super.key, this.modelDriver});

  final RequestModel? modelDriver;

  @override
  State<SelectRequestDriver> createState() => _SelectRequestDriverState();
}

class _SelectRequestDriverState extends State<SelectRequestDriver> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        context.read<GetDriverInformationsCubit>().fetchDriverData(widget.modelDriver!.userId);
        Navigator.pushNamed(context, RouteNames.driverDetailInfo);
      },
                ...
                Row(
                  ...
                  children: [
                    _rowMaker(
                      ...
                      "Bo'sh joy: ${widget.modelDriver!.passengerCount}",
                    ),
                    _rowMaker(
                      context,
                      AppIcons.getSvg(name: AppIcons.wallet, iconType: IconType.curved),
                      "${widget.modelDriver!.requestPrice} so'm",
                    ),
                  ],
                ),
                16.ph,
